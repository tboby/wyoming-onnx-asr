...
async def transcribe_wav(uri: str, wav_path: Path, model_name: str) -> Tuple[str, float]:
...
async def main():
    ...
    parser.add_argument("--uri", default="tcp://localhost:10300", help="Wyoming ASR service URI")
    parser.add_argument("--iterations", type=int, default=3, help="Number of iterations per model")
    parser.add_argument("--models", nargs="+", help="Specific models to test (default: all available)")
    ...
    try:
        ...
        # Run benchmark
        results = await run_benchmark(args.uri, args.wav_file, args.models, args.iterations)
        ...
        for model, data in results.items():
            ...
            print(f"  Successful Iterations: {data['successful_iterations']}/{args.iterations}")
